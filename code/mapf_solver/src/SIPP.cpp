...
Path SIPP::updatePath(const BasicGraph& G, const SIPPNode* goal, const State& start)
{
    ...
    while (true)
    {
        ...
        {
            int t = curr->state.timestep - start.timestep;
            path[t] = curr->state;path[t].timestep = t;
            t--;
            for (; t >= 0; t--)
            {
        ...
        else
        {
            ...
            int t = prev->state.timestep + 1 - start.timestep;
            ...
            while ( t < curr->state.timestep - start.timestep)
            {
                path[t] = State(prev->state.location, t, curr->state.orientation); // wait at prev location
                t++;
            }
            path[curr->state.timestep - start.timestep] = State(curr->state.location, curr->state.timestep - start.timestep, curr->state.orientation); // move to current location
            curr = prev;
        }
    }
    return path;
               ...
               ReservationTable& rt)
{
    ...
    while (!focal_list.empty())
    {
		...
		if (curr->goal_id == (int)goal_location.size())
		{
			Path path = updatePath(G, curr, start);
        ...
        for (int orientation = 0; orientation < 4; orientation++) // move
        {
            ...
            if (degree > std::get<1>(curr->interval) - curr->state.timestep + start.timestep) // don't have enough time to turn
                continue;
            ...
            if (h_val > INT_MAX)   // This vertex cannot reach the goal vertex
                continue;
            int min_timestep = curr->state.timestep + degree + 1 - start.timestep;
            for (auto interval : rt.getSafeIntervals(curr->state.location, location, min_timestep, std::get<1>(curr->interval) + 1))
            {
                if (curr->state.orientation < 0)
                    generate_node(interval, curr, G, location, min_timestep, -1, h_val, start);
                else
                    generate_node(interval, curr, G, location, min_timestep, orientation, h_val, start);
            }
        ...
        if(rt.use_cat) // wait to the successive interval
        {
            ...
            if (found)
            {
				if (curr->state.orientation < 0)
				{
					generate_node(interval, curr, G, location, min_timestep, -1, curr->h_val, start);
				}
				else
				{
					generate_node(interval, curr, G, location, min_timestep, orientation, curr->h_val, start);
					generate_node(interval, curr, G, location, min_timestep, (orientation + 1) % 4, curr->h_val, start);
					generate_node(interval, curr, G, location, min_timestep, (orientation + 3) % 4, curr->h_val, start);
					if (std::get<1>(curr->interval) - curr->state.timestep > 1)
						generate_node(interval, curr, G, location, min_timestep, (orientation + 2) % 4, curr->h_val, start);
				}
            }
        }
...
void SIPP::generate_node(const Interval& interval, SIPPNode* curr, const BasicGraph& G,
        int location, int min_timestep, int orientation, double h_val, const State& start)
{
    int timestep  = max(std::get<0>(interval), min_timestep);
    int wait_time = timestep - curr->state.timestep - 1 + start.timestep;; // inlcude rotate time
    ...
    // generate (maybe temporary) node
    auto next = new SIPPNode(State(location, timestep + start.timestep, orientation),
