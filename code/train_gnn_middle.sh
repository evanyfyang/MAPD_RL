...
echo "python train_mapd_gnn.py \
  ...
  --checkpoint_freq 125 \
  --global_seed 40 \
  --grid_path "/local-scratchg/yifan/2024/MAPD/MAPD_RL/code/mapf_solver/maps/Instances/large/kiva-middle.map" \
  --save_dir "${MODEL_DIR}" \
  --training \
  --task_num "${TASK_NUM}" \
  --n_envs "${PROCESS_NUM}" \
...
python train_mapd_gnn.py \
  ...
  --agent_num_lower_bound 30 \
  --agent_num_higher_bound 100 \
   ${USE_SINKHORN_FLAG} \
   ${USE_HUNGARIAN_FLAG} \
   ${USE_UNDIRECTED_FLAG} \
   ${USE_GUMBEL_HUNGARIAN_FLAG} \
